...
public class EventTestController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<EventTestDTO> getEventTest(@PathVariable final Long id) {
        return ResponseEntity.ok(eventTestService.get(id));
    }

    @PostMapping
    @ApiResponse(responseCode = "201")
    public ResponseEntity<Long> createEventTest(
    ...
    @PutMapping("/{id}")
    public ResponseEntity<Void> updateEventTest(@PathVariable final Long id,
            @RequestBody @Valid final EventTestDTO eventTestDTO) {
    ...
    public ResponseEntity<Void> deleteEventTest(@PathVariable final Long id) {
